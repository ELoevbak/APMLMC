...
def sample_fcn(samples, level, particles_per_sample, delta_t_f, epsilon, moments, stop_times, bound_L, bound_R, bound_L_type, bound_R_type, dist_fcn, scale_factor, output):
    ...
    for sample in range(samples):
        ...
        particles = [GTparticle(particle_mass, bound_L, bound_R, epsilon, delta_t_r, pos) for pos in position_random]
        if level != 0:
            particles_f = [GTparticle(particle_mass, bound_L, bound_R, epsilon, delta_t_f, pos) for pos in position_random]
            ...
            while t < stop_time-delta_t_r/2:
                if level == 0:
                    ...
                    genericParticle.make_correlated_time_steps(particles, delta_t_f, delta_t_r, epsilon, \
                                                               bound_L, bound_R, bound_L_type, bound_R_type, correlation_type="independent")
                t += delta_t_r
